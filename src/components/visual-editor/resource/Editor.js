...
class Editor {
  constructor (option) {
    ...
    this.datasourceType = option.datasourceType || 'multiple'
  ...
  init () {
    ...
    Promise.all([
      this.loadPlugin(),
      this.getModelList(),
      this.getModelDataMap(),
      this.validatePaperConfig()
    ]).then(() => {
      this.vm.onInited()
    })
  }
  ...
  bindHotKeys () {
    ...
    hotkeys('ctrl+v, command+v', async (event) => {
      let paper = this.getCurrentPaper()
      paper.onPaste(Object.values(this.clipBoard)[0])
    })
  ...
  async onModelChange (modelId) {
    let modelData = await this.getModelData(modelId)
    let paper = this.getCurrentPaper()
    paper.onModelChange(modelData)
  }
  getModelList () {
    let _this = this
    return new Promise(async (resolve, reject) => {
      if (_this.modelList.length) {
        resolve(_this.modelList)
      }

      let data = await this.service.getModelList()
      resolve(data)
    })
  }
  async getModelData (modelId) {
    if (this.modelData[modelId]) {
      return _.cloneDeep(this.modelData[modelId])
    }
    let data = await this.service.getModelData({ modelId })
    return data
  }
  getModelDataMap () {
    return new Promise((resolve, reject) => {
  ...
  getModelOptions (modelData, isTree) {
    let options = []
    const filedTypeMap = {
      'dimension': '维度',
      'measure': '指标'
    ...
    Object.keys(filedTypeMap).forEach(type => {
      ...
      if (!isTree) {
        data.forEach(item => {
          options.push(this.getFieldOption(item, type))
        })
      } else {
        options.push({
