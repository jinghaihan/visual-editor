<template>
  <div :class="`${prefixCls}-paper`" @mousedown="handleDeactivated">
    <SketchRule :adapterConfig="adapterConfig">
      ...
      >
        <div
          ref="CanvasRef"
          :id="`${prefixCls}-paper-${paper.key}`"
          :class="`${prefixCls}-paper-canvas`"
          :style="canvasStyle"
...
<script lang="ts" setup name="VisualPaper">
  ...
  function handleDeactivated(e) {
    if (e) {
      ...
      if (
        dom.classList.contains(`.${prefixCls}-paper-cell`) ||
        parents(e.target, `.${prefixCls}-paper-cell`).length ||
